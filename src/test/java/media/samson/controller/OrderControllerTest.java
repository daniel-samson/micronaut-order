...
//        var vendor = vendorRepository.create(new Vendor("Acme"));
//        assertNotEquals(null, vendor.getVendorId());
//
//        var lemonadePart = vendorPartRepository.create( new VendorPart("Lemonade", "A fizzy lemon flavored drink", new BigDecimal("0.99"), vendor));
//        assertNotEquals(null, lemonadePart.getVendorPartId());
//
//        var colaPart = vendorPartRepository.create(new VendorPart("Cola", "A fizzy soda drink", new BigDecimal("0.99"), vendor));
//        assertNotEquals(null, colaPart.getVendorPartId());
//
//        List<OrderLineItem> initialLineItems = new ArrayList<OrderLineItem>();
//        var lemonadeLineItem = new OrderLineItem(1, lemonadePart);
//        initialLineItems.add(lemonadeLineItem);
//
//        var newOrder = new Order(null, Order.Status.PENDING, initialLineItems);
//        HttpRequest<?> createRequest = HttpRequest.POST("/order", newOrder);
//        HttpResponse<Order> createdResponse = client.toBlocking().exchange(createRequest, Order.class);
//        Order order = createdResponse.getBody().get();
//
//        assertEquals(HttpStatus.CREATED, createdResponse.getStatus());
//        assertEquals(1, order.getLineItems().size());
//        assertNotEquals(null, order.getLineItems().getFirst().getOrderLineItemId());
//        assertEquals(1, order.getLineItems().getFirst().getQuantity());
//
//        var colaLineItem = new OrderLineItem(2, colaPart);
//        order.getLineItems().add(colaLineItem);
